...
class Cheng_jiao_liang_model extends CI_Model
{
    ...
    public function update_data($arr)
    {
        ...
        $chengjiaoliang_arr = array();


        if (count($query_result) < 1) {
            for ($i = 0; $i < count($arr); $i++) {
                $chengjiaoliang_arr['d' . $i] = $arr[$i]['chengjiaoliang'];
            }
            $chengjiaoliang_arr['code'] = $code;
            $this->db->insert('ten_chengjiaoliang',$chengjiaoliang_arr);
        } else {
            $sql = "update ten_chengjiaoliang set ";

            for ($i = 0; $i < count($arr); $i++) {
                array_push($chengjiaoliang_arr,'d' . $i . '=' . $arr[$i]['chengjiaoliang']);
            }
            $sql = $sql . join(',',$chengjiaoliang_arr) . " where code='$code'";
            $this->db->query($sql);
        }
    }
    ...
    {
        $this->db->query("update ten_chengjiaoliang set d60=d59,d59=d58,d58=d57,d57=d56,d56=d55,d55=d54,d54=d53,d53=d52,d52=d51,d51=d50,
