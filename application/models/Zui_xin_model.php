...
class Zui_xin_model extends CI_Model {
    ...
    public function update_data($arr)
    {
        ...
        if (count($query_result) < 1) {
            for ($i = 0; $i < count($arr); $i++) {
                $zuixin_arr['d' . $i] = $arr[$i]['zuixin'];
            }
            $zuixin_arr['code'] = $code;
            $this->db->insert('ten_zuixin',$zuixin_arr);
        } else {
            $sql = "update ten_zuixin set ";

            for ($i = 0; $i < count($arr); $i++) {
                array_unshift($zuixin_arr,'d' . $i=$arr[$i]['zuixin']);
            }
            $sql = $sql . join(',',$zuixin_arr) . " where code=$code";
            $this->db->query($sql);
        }
    }
    ...
    {
        $query = $this->db->query('select * from ten_zuixin');
        foreach ($query->result() as $row)
        {
            ...
            $this->db->query("update ten_zuixin set d0=0,d1=$d0,d2=$d1,d3=$d2,d4=$d3,d5=$d4,d6=$d5,d7=$d6,
